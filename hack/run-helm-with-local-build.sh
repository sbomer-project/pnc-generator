...
LOCAL_CHART_PATH="helm/pnc-generator-chart"
...
echo "--- Building local pnc-generator image inside of Minikube ---"
# Will have local image pnc-generator:latest in minikube
bash ./hack/build-local-pnc-generator-into-minikube.sh
...
target_name = 'pnc-generator-chart'
...
for line in lines:
    ...
    # If we are inside the pnc-generator-chart block, replace values
    if in_block:
...
helm upgrade --install sbomer-release "./$PLATFORM_DIR" \
    ...
    --set "wiremock.extraMappingConfigMaps[0]=pnc-wiremock-mappings" \
    --set pnc-generator-chart.config.pnc.apiUrl="http://sbomer-release-wiremock:8080" \
    --set pnc-generator-chart.image.repository=localhost/pnc-generator \
    --set pnc-generator-chart.image.tag=latest \
    --set pnc-generator-chart.image.pullPolicy=Never

echo "--- Forcing Rolling Restart to pick up new local image ---"
# We ignore "not found" errors in case it's the very first install
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name=pnc-generator-chart || true
